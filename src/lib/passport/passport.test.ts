...
describe("huntStateFor", () => {
	it("stays in hunting for most of the book", () => {
		expect(huntStateFor(0, 16, 0, 23)).toBe("hunting");
		expect(huntStateFor(12, 16, 12, 23)).toBe("hunting");
	});

	it("switches to closing with three or fewer wild worlds left", () => {
		expect(huntStateFor(13, 16, 13, 23)).toBe("closing");
		expect(huntStateFor(15, 16, 15, 23)).toBe("closing");
	});

	it("marks the Wild book done before the catalog is", () => {
		expect(huntStateFor(16, 16, 16, 23)).toBe("wildComplete");
		expect(huntStateFor(16, 16, 22, 23)).toBe("wildComplete");
	});

	it("only says complete when every world is found", () => {
		expect(huntStateFor(16, 16, 23, 23)).toBe("complete");
	});
	...
	it("agrees with the counters buildProgress derived", () => {
		...
		expect(progress.blurb).toBe(blurbFor("hunting", 16));
	});
});
...
describe("progress", () => {
	it("derives the wild-reachable set from slot defaults", () => {
		...
		expect(WILD_REACHABLE_WORLDS.size).toBe(16);
	});
	...
	it("covers the whole catalog with no empty pages", () => {
		...
		expect([progress.wildTotal, progress.total]).toEqual([16, WORLDS.length]);
		expect(progress.biomes.reduce((n, b) => n + b.total, 0)).toBe(WORLDS.length);
		expect(progress.biomes.reduce((n, b) => n + b.wildTotal, 0)).toBe(16);
	...
	it("lets a free user complete the Wild book but not the catalog", () => {
		...
		expect([progress.wildFound, progress.wildTotal]).toEqual([16, 16]);
		expect([progress.found, progress.total]).toEqual([16, 23]);
	});
	...
	it("fills the remaining pages by chartering, without moving the wild score", () => {
		...
		const progress = buildProgress(book);
		expect([progress.found, progress.total]).toEqual([23, 23]);
		expect(progress.wildFound).toBe(16);
