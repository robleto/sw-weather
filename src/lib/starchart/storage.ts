 ...
 * or world removed in a later release must not break the whole chart — the
 * unknown entry is discarded and that slot falls back to its default.
 */
const sanitize = (raw: unknown): StarChartOverrides => {
	if (!raw || typeof raw !== "object") return {};

	const result: StarChartOverrides = {};
...
export const loadOverrides = (): StarChartOverrides => {
...
export const saveOverrides = (overrides: StarChartOverrides): void => {
	if (typeof window === "undefined") return;
	try {
		window.localStorage.setItem(STORAGE_KEY, JSON.stringify(overrides));
	} catch {
		// Storage full or blocked — the in-memory chart still works for
		// this session.
	}
};
